...
costFunction <- function(y_hist, N, w, dt) {
    ...
    halflen <- ceiling(length(y_hist) / 2)
    remlen <- length(y_hist) - halflen
    addleft <- fftKernel(c(numeric(remlen), y_hist[1:halflen]), w / dt)
    addright <- fftKernel(c(y_hist[(halflen+1):length(y_hist)], numeric(remlen)), w / dt)
    yh <- yh + c(rev(addleft[1:halflen]), numeric(remlen)) + c(numeric(remlen), rev(addright[(halflen + 1):length(addright)]))
